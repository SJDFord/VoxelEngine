...
public:
    ...
    vector<std::shared_ptr<MeshBuffer>>    meshes;
    string directory;
        ...
        std::vector<Transform3> instanceTransformations)
    {
        for(unsigned int i = 0; i < meshes.size(); i++) {
            this->meshRenderer->render(screenDimensions, meshes[i], shader, camera, lighting, instanceTransformations);
        }
    }
...
private:
    ...
    std::shared_ptr<MeshBuffer> processMesh(aiMesh *mesh, const aiScene *scene)
    {
        // data to fill
        ...
        fprintf(stdout, "Loaded mesh %i, %i, %i\n", vertices.size(), indices.size(), textures.size());

        return std::make_shared<MeshBuffer>(m);
    }
    ...
    vector<std::shared_ptr<Texture>> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
    {
        ...
        for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {
                fprintf(stdout, "Comparing %s %s\n", textures_loaded[j]->getIdentifier().c_str(), str.C_Str());
                if(std::strcmp(textures_loaded[j]->getIdentifier().c_str(), str.C_Str()) == 0)
                {
            ...
            {   // if texture hasn't been loaded already, load it
                ...
                std::shared_ptr<Texture> texture = textureLoader.loadFromFile(filepath, typeName);
