...
void UniformChunk::render(const ICamera& camera, const ILightSource& light, const Shader& shader) const
{
	if (_chunkRenderer == nullptr) return;
	_chunkRenderer->render(shader, camera, *_chunkGeometry, light);
}
